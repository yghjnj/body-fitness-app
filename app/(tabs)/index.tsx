  ...
  return (
    ...
    >
      ...
      <Card style={styles.quickCard}>
        <Text style={styles.greeting}>身体数据速查</Text>
        <Text style={styles.subtitle}>填写以下数据，快速查看各项身体指标</Text>
        <View style={styles.inputGrid}>
          ...
          <View style={styles.inputHalf}>
            ...
            <TextInput
              style={styles.dataInput}
              value={height}
              onChangeText={(v) => { setHeight(v); settings.updateSettings({ heightCm: parseFloat(v) || 170 }); }}
              keyboardType="decimal-pad"
              placeholder="170"
              placeholderTextColor={Colors.textTertiary}
            />
          ...
          <View style={styles.inputHalf}>
            ...
            <TextInput
              style={styles.dataInput}
              value={age}
              onChangeText={(v) => { setAge(v); settings.updateSettings({ age: parseInt(v) || 25 }); }}
          ...
          <View style={styles.inputHalf}>
            ...
            <View style={styles.genderRow}>
              <TouchableOpacity
                ...
                onPress={() => { setGender('male'); settings.updateSettings({ gender: 'male' }); }}
              >
                <Text style={[styles.genderBtnText, gender === 'male' && styles.genderBtnTextActive]}>男</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.genderBtn, gender === 'female' && styles.genderBtnActive]}
                onPress={() => { setGender('female'); settings.updateSettings({ gender: 'female' }); }}
              >
      ...
      {/* BMI Result */}
      {bmiResult && <BMICard weightKg={weightNum} heightCm={heightNum} />}

      {/* VIP Banner — shows for free users */}
      <VIPBanner />
      ...
      <View style={styles.actionGrid}>
        <ActionCard
          ...
          subtitle="记录每日饮水"
          color="#3B82F6"
          onPress={() => router.push('/(modals)/add-water')}
        />
        <ActionCard
          icon="restaurant"
          title="饮食记录"
        ...
        <ActionCard
          ...
          subtitle="记录身材变化"
          color="#8B5CF6"
          onPress={() => router.push('/(modals)/progress-photo')}
        />
        <ActionCard
          icon="calculator"
          title="BMR/TDEE"
        ...
        <ActionCard
          icon="bulb"
          title="AI 饮食建议"
          subtitle={isVIP ? '智能饮食推荐' : 'VIP 专属'}
          color={Colors.vipGold}
          onPress={() => isVIP ? router.push('/(vip)/ai-diet') : router.push('/(modals)/paywall')}
          locked={!isVIP}
        />
        <ActionCard
          icon="analytics"
          title="高级分析"
          subtitle={isVIP ? '趋势+预测' : 'VIP 专属'}
          color={Colors.vipGold}
          onPress={() => isVIP ? router.push('/(vip)/advanced-analytics') : router.push('/(modals)/paywall')}
          locked={!isVIP}
        />
      </View>
